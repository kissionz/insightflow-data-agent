...
“今年、本月、本季度、本周”等当前周期截止到当天；同比和环比使用独立参数化的
同进度历史窗口。多步分析产生的全部 SQL 会在证据链中按 `SQL 1`、`SQL 2`
顺序独立展示。

数字属性不需要逐个重复创建指标。只要本体为 `NUMBER` 属性设置了非 `NONE`
默认聚合，它就会作为属性型度量返回给 Montane，并由 IR 按 `SUM/AVG/MIN/MAX`
确定性编译；复杂公式和过滤口径继续使用指标中心中的正式 Metric。指标与对象编辑
分离，但每个指标仍声明一个事实对象。复合指标可以引用同一事实对象下的基础或复合
指标形成依赖图，例如先定义“毛利额 = 销售额 - 成本额”，再定义
“毛利率 = 毛利额 / 销售额 × 100”。当前版本会拒绝跨事实对象依赖和循环引用。

Query IR v2 支持日、周、月、季度、年时间粒度，环比和同比取数，受控加减乘除、
比率、排名、累计值与移动平均。临时派生计算和正式复合指标都保留完整依赖图，
由 IR 递归展开并使用 `NULLIF` 保护除零。复杂筛选使用最多四层的
`AND/OR/NOT` 逻辑树。“各SPU销售额大于3000万、毛利率大于75%”等指标阈值
使用独立的聚合后筛选 IR，编译器先按维度汇总，再在分层结果上参数化筛选；指标
条件不能降级为明细字段 `WHERE`。问题框架中的每个指标阈值必须被最终 IR 完整覆盖。
规则引擎会检查数字属性可加性、跨事实对象混算和关系扇出；半可加指标不能跨时间
直接求和，一对多或多对多路径可能放大事实行时会在执行前拒绝查询。
