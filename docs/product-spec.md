...
- 发布前必须通过对象映射、对象标识、指标表达式、关系端点和危险 SQL 校验。
...
| 维度 | 配置 | 说明 |
|---|---|---|
| 数据语义 | 标识符、维度、枚举、时间、地理、金额、数量、布尔 | 描述字段表达什么数据 |
| 身份角色 | 普通属性、对象标识、业务唯一键 | 描述字段如何标识业务对象 |
| 关系参与 | 关联键 | 由 Relation 的源和目标字段动态计算，不重复存储 |
| 使用策略 | 分析可用、仅明细、完全隐藏 | 决定索引、Montane 上下文和结果展示 |

一个对象至少需要一个“对象标识”属性，并允许多个属性共同构成复合对象标识。“业务唯一键”用于订单号、客户编码等稳定业务编码。关联键必须通过关系编辑器维护；删除关系后，相应属性的关联键标识自动消失。

旧版本快照中的对象级 `primaryKey` 在读取时自动迁移为属性级 `OBJECT_IDENTIFIER`，不要求用户重新建模。

### 8.10 对象编辑清单

- 基础：业务名称、显示名称、描述、分类、粒度、同义词。
- 属性：语义类型、身份角色、动态关联键、可见性、描述、同义词、默认明细展示、导出权限。
