...
- 基础：业务名称、对象类型、描述、分类、结构化行级粒度、同义词。
- 属性：字段含义、唯一约束、关联目标、数字规则、值定位权限、可见性、描述、同义词、默认明细展示、导出权限。
